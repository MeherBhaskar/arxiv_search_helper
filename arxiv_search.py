...
def create_llm():
    ...
    class FallbackLLM(LLM):
        def _call(self, prompt: str, stop: Optional[List[str]] = None) -> str:
            return f"Based on the research papers, the query about '{prompt}' relates to several scientific works. " \
...
def rank_papers_by_relevance(query, papers, embedding_model):
    ...
    try:
        ...
        ranked_papers = [paper for paper, score in sorted(paper_scores, key=lambda x: x[1], reverse=True)]
